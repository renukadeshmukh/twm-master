    ...
    $.addFlex = function (t, p) {
        ...
        var g = {
            ...
            doSearch: function () {
                if (p.CustomSearch == null) {
                    ...
                    p.query = '';
                    p.qtype = '';
                    $('input[name=q]', g.sDiv).each(function (index, value) {
                        p.qtype = p.qtype + 'Ω' + $(this).attr('data-field');
                        p.query = p.query + 'Ω' + $(this).attr('data-field') + '§' + $(this).val();
                    });
                }
                p.newp = 1;
                this.populate();
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                if (p.CustomSearch != null) {
                    var jObj = p.CustomSearch();
                    var div = $('<div class="sDiv2"></div>');
                    div.append(jObj);
                    div.append('<div id="searchOptions"><table><tr><td><input type="button" value="Search" /></td><td>&nbsp;&nbsp;</td><td><input type="button" value="Clear" /></td><td>&nbsp;&nbsp;</td><td></td></tr></table></div>');
                    /*<input type="button" value="Delete" title="Delete all records for givern search criteria, If criteria is empty then all records will be deleted" />*/

                    $('div#searchOptions input[value=Search]', div).click(function () { p.action = 'search'; g.doSearch(); });
                    $('div#searchOptions input[value=Clear]', div).click(function () { p.action = 'search'; $('input[name=q]', div).val(''); });
                    /*$('div#searchOptions input[value=Delete]', div).click(function () {
                    $('<div  title="Confirm Delete" style="height:auto;width:100%"><div class="warningIcon" style="float:left; margin:0px 7px 0px 0px;"></div><div style="float:left;margin-top: 5px;width:90%;">Are you sure you want to delete all records for given criteria</div></div>')
                    .dialog({
                    modal: true,
                    ...
                    Ok: function () { p.action = 'delete'; g.doSearch(); $(this).dialog('destroy').remove(); }
                    },
                    resizable: false,
                    maxWidth: 500,
                    width: 500
